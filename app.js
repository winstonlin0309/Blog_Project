...
async function main() {
  ...
  app.get("/", function(req, res) {
    Blog.find({}).then((foundItem) => {
      res.render("home", {
        pageTitle: 'Home',
        startingContent: homeStartingContent,
        contentPosts : foundItem
      });
    }).catch((err) => {
      console.log(err);
    })
  ...
  app.get("/posts/:contentTitle", function(req, res) {
    let searchTitle = _.lowerCase(req.params.contentTitle);
    
    Blog.find({title : searchTitle}).then((foundBlog) => {
      if(foundBlog != null) {
        res.render("post", {
          pageTitle: foundBlog.title,
          startingContent: foundBlog.content
        });
      }
